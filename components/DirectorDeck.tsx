...
import { Grid2X2, Grid3X3, Zap, Layers } from 'lucide-react';
...
export const DirectorDeck: React.FC<DirectorDeckProps> = ({
  ...
  const addPreset = (value: string) => {
      const prefix = prompt.trim().length > 0 ? prompt.trim() + ", " : "";
      setPrompt(prefix + value);
  };
        ...
        <div className="grid grid-cols-4 gap-1">
             {Object.values(ImageSize).map((size) => (
                <button
                    ...
                    className={`text-[9px] h-7 border rounded-[1px] font-mono transition-colors flex items-center justify-center ${
                        imageSize === size 
                        ? 'border-zinc-600 text-cine-accent bg-zinc-800' 
                        : 'border-zinc-800 text-zinc-600 hover:border-zinc-600 hover:text-zinc-400 bg-black'
                    }`}
                >
                    {size}
